...
from comet.modules.feedforward import Gate


class CtxRegressionMetric(CometModel):
    """RegressionMetric:
    ...
    def __init__(
        ...
        self.gate = Gate(
            in_dim=self.encoder.output_units * 3,
            out_dim=1,
            dropout=self.hparams.dropout,
    ...
    def prepare_sample(
        ...
        src_ctx_inputs = self.encoder.prepare_sample(sample["src_ctx"])
        ref_ctx_inputs = self.encoder.prepare_sample(sample["ref_ctx"])

        src_inputs = {"src_" + k: v for k, v in src_inputs.items()}
        mt_inputs = {"mt_" + k: v for k, v in mt_inputs.items()}
        ref_inputs = {"ref_" + k: v for k, v in ref_inputs.items()}
        src_ctx_inputs = {"src_ctx_" + k: v for k, v in src_ctx_inputs.items()}
        ref_ctx_inputs = {"ref_ctx_" + k: v for k, v in ref_ctx_inputs.items()}
        inputs = {**src_inputs, **mt_inputs, **ref_inputs, **src_ctx_inputs, **ref_ctx_inputs}
    ...
    def forward(
        ...
        src_ctx_input_ids: torch.tensor,
        src_ctx_attention_mask: torch.tensor,
        ref_ctx_input_ids: torch.tensor,
        ref_ctx_attention_mask: torch.tensor,
        **kwargs
    ) -> Dict[str, torch.Tensor]:
        ...
        src_ctx_sentemb = self.get_sentence_embedding(src_ctx_input_ids, src_ctx_attention_mask)
        ref_ctx_sentemb = self.get_sentence_embedding(ref_ctx_input_ids, ref_ctx_attention_mask)

        gate = self.gate(torch.cat((mt_sentemb, ref_sentemb, ref_ctx_sentemb), dim=1))
        # print(gate)

        srcctx_sentemb = gate * src_sentemb + (1 - gate) * src_ctx_sentemb
        refctx_sentemb = gate * ref_sentemb + (1 - gate) * ref_ctx_sentemb
        mtctx_sentemb = gate * mt_sentemb + (1 - gate) * ref_ctx_sentemb

        return self.estimate(srcctx_sentemb, mtctx_sentemb, refctx_sentemb)
    ...
    def read_csv(self, path: str) -> List[dict]:
        ...
        """
        df = pd.read_csv(path)
        df = df[["src", "mt", "ref", "score", "src_ctx", "ref_ctx"]]
        df["src"] = df["src"].astype(str)
        df["mt"] = df["mt"].astype(str)
        df["ref"] = df["ref"].astype(str)
        df["src_ctx"] = df["src_ctx"].astype(str)
        df["ref_ctx"] = df["ref_ctx"].astype(str)
