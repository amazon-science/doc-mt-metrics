...
class CtxReferencelessRegression(RegressionMetric):
    """ReferencelessRegression:
    ...
    def prepare_sample(
        ...
        ctx_inputs = self.encoder.prepare_sample(sample["ctx"])

        src_inputs = {"src_" + k: v for k, v in src_inputs.items()}
        mt_inputs = {"mt_" + k: v for k, v in mt_inputs.items()}
        ctx_inputs = {"ctx_" + k: v for k, v in ctx_inputs.items()}
        inputs = {**src_inputs, **mt_inputs, **ctx_inputs}
    ...
    def forward(
        ...
        **kwargs
        ...
        ctx_sentemb = self.get_sentence_embedding(ctx_input_ids, ctx_attention_mask)

        gate = self.gate(torch.cat((src_sentemb, ctx_sentemb), dim=1))
        # gate = self.gate(src_sentemb)
        # print(gate)

        srcctx_sentemb = gate*src_sentemb + (1-gate)*ctx_sentemb

        diff_src = torch.abs(mt_sentemb - srcctx_sentemb)
        prod_src = mt_sentemb * srcctx_sentemb

        embedded_sequences = torch.cat(
            (mt_sentemb, srcctx_sentemb, prod_src, diff_src), dim=1
        )
