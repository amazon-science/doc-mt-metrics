...
def collate_idf_with_context(arr, tokenizer, idf_dict, device="cuda:0"):
    ...
    for sent in arr:
        ...
        separators = np.where(np.array(sent) == tokenizer.sep_token_id)[0].tolist()
        idf_weight = [idf_dict[i] for i in sent]
        if len(separators) > 1:
            # if there are more than one find where the last sentence starts
            ind = separators[-2]
            context_weight = [0] * (ind + 1) + [1.0] * (len(sent) - ind - 1)
        else:
            context_weight = [1] * len(sent)
        # combine idf weights with context weights
        final_idf_weight = np.logical_and(idf_weight, context_weight).astype(float).tolist()
        idf_weights.append(final_idf_weight)
